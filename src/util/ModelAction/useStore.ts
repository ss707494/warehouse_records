...
const graphQLMutate = () => async (mutation: any, params: any, option: any) => {
  console.log(mutation)
  console.log(params)
  console.log(option)
}
const graphQLQuery = () => async (mutation: any, params: any, option: any) => {
  console.log(mutation)
  console.log(params)
  console.log(option)
}
...
export const useStoreModel: UseModelState = (model, key?: string | [string, string]) => {
  ...
  const query: GraphqlQuery = useCallback(async (query, params, option) => {
    setLoad(query, true)
    const res = await graphQLQuery()(query, params, option).catch((e: any) => {
      setError(query, e)
    }).finally(() => {
      setLoad(query, false)
    }) as any
